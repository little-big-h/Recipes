...
- **XiaoYanZi Tomato Hot Pot** (小燕子番茄火锅), Clementi Ave 3 — tomato-broth steamboat chain, vegetarian-workable: confirmed the tomato broth itself is fine to order (not a hidden meat stock, per the 2026-09-11 visit) and pick tofu/mushroom/veg items to cook in it. Skip the dipping-sauce sesame/peanut oil to keep the meal lean; deliberately add a carb source (noodles/rice) since hotpot easily ends up protein/veg-heavy and carb-light for Holger. Per-item nutrition (Plate & Shoot weighs each hotpot ingredient separately, not as one dish) in `docs/MEAL_LOG_PRODUCTS.md`.
