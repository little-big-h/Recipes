...
- **`.foodnoms` files are generated by `tools/js`, always.** `node tools/js/cli.js build <recipe.json>` writes the file locally — USDA lookup, nutrition arithmetic and file assembly all happen on this machine. Never hand-build the JSON, and never rely on the Wolfram endpoint to produce the file. See `tools/js/README.md`.
